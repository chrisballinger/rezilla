...
//             Last modification: 2004-12-22 17:06:22
...
// ---------------
//    		
//   We keep track of which "node" (horiz/vert coordinate between the
//   pixels) was clicked on and which node is closest to the mouse at any
//   point in time. The user can "back-off" by tracing the path backwards,
//   so mistakes are easier to correct.
...
CIconLassoAction::CIconLassoAction( const SPaintAction &inAction ) 
...
void
CIconLassoAction::HandleMouseDown( const SMouseDownEvent &inEvent )
{
	SInt32	rowHit, colHit;
	
	CIcon_EditorView *theCanvas = mSettings.theCanvas;
	
	// If the user clicks within the current selection, we will instantiate
	// a move tool and kill ourselves.
...
void
CIconLassoAction::MouseInitial( const SMouseDownEvent &inEvent, SInt32, SInt32 )
{
	CIcon_EditorView *theCanvas = mSettings.theCanvas;
...
void
CIconLassoAction::MouseStillDown( const SMouseDownEvent &, 
										...
										SInt32, SInt32 )
{
	...
	// If the node is the same as the previous one, do nothing but animate
	// (haven't moved)
	...
	// If the node is the same as the second previous one, erase the previous line
	if ( mNumPoints > 1 )
	{
...
// 	if ( mNumPoints > 1 ) {
// 		// Draw a single pixel line in the canvas from the previous
// 		// point to the current point. The line needs to be between
// 		// pixels.
...
Boolean
CIconLassoAction::MouseFinal( const SMouseDownEvent &, 
									Point, Point,
									SInt32, SInt32,
									SInt32, SInt32 )
{	
...
// 	AddPointToArray
// ---------------------------------------------------------------------------
// 	Maps the point to the image bounds and always returns true
// 	unless the buffer is full.

Boolean
CIconLassoAction::AddPointToArray( SInt32 newCol, SInt32 newRow )
{
	if ( mNumPoints >= kMaxPoints - 1 ) return(false);		// Sorry, we're full
															// Save 1 slot to close the polygon
	
	// Bounds check the incoming values because the mouse tracking returns
	// #'s outside of the canvas
...
RgnHandle
CIconLassoAction::ConvertArrayToRegion()
{
	...
	// In order to be valid, we need at least 3 points and they may not be
	// linear because there is no enclosed area.
	...
	::CloseRgn( outRegion );		// should check for errors
...
void
CIconLassoAction::RestrictRegion( RgnHandle selectionRgn )
{
	...
	try
	{
		// Copy the current buffer to the scratch buffer, mapping all
		// non-background pixels to black.
		...
		// We'll setup a bitmap structure and use it instead. (apparently,
		// the hi bits of rowBytes screw it up 'cause it doesn't work)
...
void
CIconLassoAction::CopyAndDownSample( COffscreen *source, COffscreen *dest, RgnHandle selectionRgn )
{
	StGWorldSaver		save1;
	
	// Copy the source to the destination, changing all non-background
	// colors to black.
...
Boolean
CIconLassoAction::IsArrayLinear()
{
	if ( mNumPoints < 3 ) 
		return( true );
...
// 	routine is only called when the user double-clicks on the lasso tool to
// 	"select all".
...
void
CIconLassoAction::FindClosestNode( const Point &inLocal, SInt32 *outH, SInt32 *outV )
{
...
void
CIconLassoAction::RemovePointFromArray()
{
...
void
CIconLassoAction::DrawSegment( SInt32 inSegmentNo )
{
